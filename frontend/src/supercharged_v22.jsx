...
.outer{display:flex;justify-content:center;padding:20px 0 32px;background:#1C1C1E;min-height:100vh}
.phone{width:355px;height:769px;border-radius:44px;border:1px solid rgba(255,255,255,0.08);overflow:hidden;box-shadow:0 40px 80px rgba(0,0,0,0.6);display:flex;flex-direction:column;background:#EDECEA;position:relative;font-family:'DM Sans',system-ui,sans-serif}
...
/* ── Change 11: Profile completeness bar ── */
...
/* ── Change 12B: Edit recency label ── */
.edit-recency-lbl{position:absolute;top:58px;right:10px;z-index:10;font-size:9px;font-weight:500;color:rgba(255,254,253,0.6);font-family:'DM Sans',system-ui,sans-serif;text-align:right;pointer-events:none;white-space:nowrap}

/* ── Change 13: Romantic card ── */
...
.romantic-tag-primary{background:#F9C0D0;color:#8C1A3A;border-radius:999px;padding:6px 12px;font-size:12px;font-weight:600}
.romantic-tag-secondary{background:rgba(176,35,80,0.08);border:1px solid rgba(176,35,80,0.22);color:#B02350;border-radius:999px;padding:5px 11px;font-size:12px}
.romantic-note{font-size:12px;color:#B02350;font-style:italic;line-height:1.5;opacity:0.85}
.tag-chip.appearance{border-color:#B02350;color:#B02350}

/* ── Change 13: Intent toggle ── */
...
/* ── v18 Change 2 + v19 Change 1B: Work card (warm amber) ── */
.pc-work{background:#FDF3E7}
.pc-work .p-ico{background:#E8C28A}
.pc-work .p-ico svg{stroke:#6B3A10}
.pc-work .p-label{color:#8C4E14}
.pc-work .p-preview-name{color:#6B3A10}
.pc-work .p-preview-rest{color:#8C4E14}
.pc-work .p-chev svg{stroke:#8C4E14}
.work-expanded{padding:0 16px 16px}
.work-item{display:flex;align-items:flex-start;gap:10px;padding:10px 0;border-bottom:0.5px solid rgba(140,78,20,0.12)}
.work-item:last-child{border-bottom:none;padding-bottom:0}
.work-dot{width:6px;height:6px;border-radius:50%;background:#8C4E14;margin-top:5px;flex-shrink:0}
.work-title{font-size:13px;font-weight:500;color:#6B3A10;margin-bottom:2px}
.work-org{font-size:11px;color:#8C4E14;opacity:0.75}
.work-badge{display:inline-block;font-size:9px;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;padding:2px 7px;border-radius:999px;margin-top:3px;background:#E8C28A;color:#6B3A10}

/* ── v18 Change 3: Visibility toggle + how-you-show-up grid ── */
...
/* ── v19 Change 3A: Howup field row (tappable) ── */
...
/* ── v19 Change 3A: Howup picker sheet (slides from right, z-index 75) ── */
.howup-picker{position:absolute;inset:0;z-index:75;background:#EDECEA;transform:translateX(100%);transition:transform 0.34s cubic-bezier(0.22,1,0.36,1);display:flex;flex-direction:column;pointer-events:none}
...
// ─── v18 Change 3 + v19 Change 2/3: How you show up — field config ──────────
const HOWUP_FIELDS = ['Pronouns','Gender','Sexuality','InterestedIn','Height','Ethnicity','Religion','LookingFor','RelStyle'];
const HOWUP_LABELS = {
  Pronouns:'Pronouns', Gender:'Gender', Sexuality:'Sexuality',
  InterestedIn:'Interested in', Height:'Height', Ethnicity:'Ethnicity',
  Religion:'Religion', LookingFor:'Looking for', RelStyle:'Relationship style'
};
const HOWUP_DEFAULT_VIS = {
  Pronouns:false, Gender:false, Sexuality:false, InterestedIn:false,
  Height:true, Ethnicity:false, Religion:false,
  LookingFor:true, RelStyle:true
};
// ─── v19 Change 3B: Option banks ────────────────────────────────────────────
const HOWUP_OPTIONS = {
  Pronouns:     ['she/her','he/him','they/them','she/they','he/they','ze/zir','xe/xem'],
  Gender:       ['Woman','Man','Non-binary','Genderqueer','Genderfluid','Agender','Transgender woman','Transgender man','Two-spirit'],
  Sexuality:    ['Straight','Gay','Lesbian','Bisexual','Pansexual','Queer','Asexual','Demisexual','Questioning'],
  InterestedIn: ['Men','Women','Non-binary people','Everyone'],
  Height:       ["4'10","4'11","5'0","5'1","5'2","5'3","5'4","5'5","5'6","5'7","5'8","5'9","5'10","5'11","6'0","6'1","6'2","6'3","6'4","6'5+"],
  Ethnicity:    ['East Asian','South Asian','Southeast Asian','Black / African','Mixed','Latino / Hispanic','Middle Eastern','White / Caucasian','Indigenous','Prefer not to say'],
  Religion:     ['Agnostic','Atheist','Buddhist','Catholic','Christian','Hindu','Jewish','Muslim','Sikh','Spiritual','Prefer not to say'],
  LookingFor:   ['Something real','Long-term relationship','Casual dating','New friends','Not sure yet','Open to anything'],
  RelStyle:     ['Monogamous','Ethical non-monogamy','Open relationship','Polyamorous','Figuring it out']
};
const HOWUP_MAX = {
  Pronouns:4, Gender:1, Sexuality:1, InterestedIn:3,
  Height:1, Ethnicity:1, Religion:1, LookingFor:1, RelStyle:1
};
...
function getHobbyIcon(name) {
  ...
  const s = 'stroke="#633806" fill="none" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"';
...
function TagInput({ type, chips, setChips, placeholder, chipClass }) {
...
export default function SuperchargedProfile() {
  ...
  const [profile, setProfile] = useState(() => {
    // Howup field defaults — read from localStorage with sensible visibility defaults
    const howupVals = {};
    const howupVis = {};
    HOWUP_FIELDS.forEach(f => {
      howupVals[f] = LS.get('sc_howup_' + f.toLowerCase()) || '';
      const stored = LS.get('sc_howup_vis_' + f.toLowerCase());
      howupVis[f] = stored !== null && stored !== undefined ? stored === 'true' : HOWUP_DEFAULT_VIS[f];
    });
    return {
      ...
      intent:    LS.get('sc_intent') || 'build',
      lastUpdated: LS.get('sc_last_updated'),
      // v18 Change 2: Work
      workCurrent:  LS.get('sc_work_current')  || 'Co-founder & CEO, Supercharged',
      workPrev:     LS.get('sc_work_prev')     || 'Incoming SBA, McKinsey & Company',
      workIndustry: LS.get('sc_work_industry') || 'Tech / Consulting',
      // v18 Change 3: How you show up
      howupVals,
      howupVis,
    };
  });

  // Seed defaults to localStorage on first load
  useEffect(() => {
    const seeds = [
      ['sc_work_current','Co-founder & CEO, Supercharged'],
      ['sc_work_prev','Incoming SBA, McKinsey & Company'],
      ['sc_work_industry','Tech / Consulting'],
      // v19 Change 4: rich howup defaults — values + visibility
      ['sc_howup_height',"5'11"],          ['sc_howup_vis_height','true'],
      ['sc_howup_pronouns','he/him'],      ['sc_howup_vis_pronouns','false'],
      ['sc_howup_lookingfor','Something real'], ['sc_howup_vis_lookingfor','true'],
      ['sc_howup_relstyle','Monogamous'],  ['sc_howup_vis_relstyle','true'],
      ['sc_howup_sexuality','Straight'],   ['sc_howup_vis_sexuality','false'],
      ['sc_howup_gender','Man'],           ['sc_howup_vis_gender','false'],
    ];
    seeds.forEach(([k,v]) => { if (LS.get(k) === null || LS.get(k) === undefined) LS.set(k, v); });
    // After seeding, if the profile state is missing these (first run), refresh it from LS
    setProfile(p => {
      const vals = { ...p.howupVals };
      const vis  = { ...p.howupVis };
      HOWUP_FIELDS.forEach(f => {
        const v = LS.get('sc_howup_' + f.toLowerCase());
        if (v !== null && v !== undefined && !vals[f]) vals[f] = v;
        const sv = LS.get('sc_howup_vis_' + f.toLowerCase());
        if (sv !== null && sv !== undefined) vis[f] = sv === 'true';
      });
      return { ...p, howupVals: vals, howupVis: vis };
    });
  }, []);

  // Seed last_updated to ~3 days ago on first load
  useEffect(() => {
    if (!LS.get('sc_last_updated')) {
      const ts = String(Date.now() - 3*86400000);
      LS.set('sc_last_updated', ts);
      setProfile(p => ({...p, lastUpdated: ts}));
    }
  }, []);
  ...
  // Relative time helper (Change 12B)
  ...
  // Profile completeness (Change 11)
  const completenessSteps = [
    ...
    { id:'aboutme',    label:'Fill in music, hobbies & societies', ctaLabel:'Edit', cta:() => openEdit(),                  done: profile.music.length>0 && profile.hobbies.length>0 && profile.campus.length>0 },
    { id:'intent',     label:'Set your "Here to" intent',     ctaLabel:'Edit',      cta:() => openEdit(),                  done: true },
    { id:'connection', label:'Make your first connection',    ctaLabel:'Explore',   cta:() => setConnOpen(true),          done: true },
  ];
  ...
  const isRomantic = profile.intent === 'romantic';
  ...
  const openEdit = () => {
    setEditData({
      ...
      // v18 Change 2 — Work
      workCurrent: profile.workCurrent, workPrev: profile.workPrev, workIndustry: profile.workIndustry,
      // v18 Change 3 — Howup vals + vis (snapshot)
      howupVals: { ...profile.howupVals },
      howupVis:  { ...profile.howupVis },
    });
    setEditOpen(true);
  };

  // ── v19 Change 3: Howup picker state + handlers ──
  const [howupPicker, setHowupPicker] = useState({
    open: false,
    field: null,
    selected: [],   // array of selected option strings from the bank
    other: '',      // free-text "something else"
    visible: false,
  });
  ...
  const openHowUpPicker = (field) => {
    ...
    const stored = editData.howupVals[field] || '';
    const allVals = stored ? stored.split(', ').map(s => s.trim()).filter(Boolean) : [];
    const selected = allVals.filter(v => opts.includes(v));
    const otherVals = allVals.filter(v => !opts.includes(v));
    setHowupPicker({
      open: true,
      field,
      selected,
      other: otherVals.join(', '),
      visible: editData.howupVis[field],
    });
  };

  const closeHowUpPicker = () => setHowupPicker(p => ({ ...p, open: false, field: null }));

  const toggleHowUpOption = (opt) => {
    setHowupPicker(p => {
      const field = p.field;
      const max = HOWUP_MAX[field] || 1;
      if (p.selected.includes(opt)) {
        return { ...p, selected: p.selected.filter(x => x !== opt) };
      }
      if (max === 1) return { ...p, selected: [opt] };
      if (p.selected.length >= max) return p; // silently ignore at cap
      return { ...p, selected: [...p.selected, opt] };
    });
  };

  const toggleHowUpPickerVis = () => setHowupPicker(p => ({ ...p, visible: !p.visible }));

  const saveHowUpPicker = () => {
    const { field, selected, other, visible } = howupPicker;
    if (!field) return;
    const finalVals = [...selected];
    const otherTrim = other.trim();
    if (otherTrim) finalVals.push(otherTrim);
    const finalVal = finalVals.join(', ');
    // Persist live
    LS.set('sc_howup_' + field.toLowerCase(), finalVal);
    LS.set('sc_howup_vis_' + field.toLowerCase(), String(visible));
    // Update editData (still in edit panel)
    setEditData(d => ({
      ...d,
      howupVals: { ...d.howupVals, [field]: finalVal },
      howupVis:  { ...d.howupVis,  [field]: visible },
    }));
    // Update profile so the card preview/grid in the profile view reacts immediately
    setProfile(p => ({
      ...p,
      howupVals: { ...p.howupVals, [field]: finalVal },
      howupVis:  { ...p.howupVis,  [field]: visible },
    }));
    closeHowUpPicker();
  };
  ...
  const saveEdit = () => {
    ...
    const p = {
      ...
      lastUpdated: ts,
      // v18 Change 2 — Work
      workCurrent: editData.workCurrent, workPrev: editData.workPrev, workIndustry: editData.workIndustry,
      // v18 Change 3 — Howup
      howupVals: editData.howupVals,
      howupVis:  editData.howupVis,
    };
    ...
    LS.set('sc_intent', p.intent);
    LS.set('sc_last_updated', ts);
    LS.set('sc_work_current', p.workCurrent);
    LS.set('sc_work_prev', p.workPrev);
    LS.set('sc_work_industry', p.workIndustry);
    HOWUP_FIELDS.forEach(f => {
      LS.set('sc_howup_' + f.toLowerCase(), p.howupVals[f] || '');
      LS.set('sc_howup_vis_' + f.toLowerCase(), String(p.howupVis[f]));
    });
    setProfile(p);
    setEditOpen(false);
  };
      ...
      <div className="phone">
          ...
          <div className="sbar-top">
            ...
            <div style={{display:'flex',gap:5}}>
              <svg width="18" height="13" viewBox="0 0 18 13" fill="none">
                <rect x="0" y="8" width="3" height="5" rx="0.7" fill="#1A1A1A"/>
                <rect x="5" y="5.5" width="3" height="7.5" rx="0.7" fill="#1A1A1A"/>
                <rect x="10" y="3" width="3" height="10" rx="0.7" fill="#1A1A1A"/>
                <rect x="15" y="0" width="3" height="13" rx="0.7" fill="#1A1A1A" opacity="0.3"/>
              </svg>
              <svg width="27" height="13" viewBox="0 0 27 13" fill="none">
                <rect x="0.5" y="0.5" width="22" height="12" rx="3.5" stroke="#1A1A1A" strokeOpacity="0.35"/>
                <rect x="2" y="2" width="17" height="9" rx="2.5" fill="#1A1A1A"/>
              </svg>
            </div>
          </div>
          <div className="url-row">
            <svg width="10" height="13" viewBox="0 0 10 13" fill="#6B6B6B">
              <path d="M5 0C3.07 0 1.5 1.57 1.5 3.5V5H1C.45 5 0 5.45 0 6v6c0 .55.45 1 1 1h8c.55 0 1-.45 1-1V6c0-.55-.45-1-1-1H8.5V3.5C8.5 1.57 6.93 0 5 0Zm0 1.5c1.1 0 2 .9 2 2V5H3V3.5c0-1.1.9-2 2-2Z"/>
            </svg>
            <span className="url-text">supercharged.app/alex</span>
          </div>
        </div>
        ...
        <div className="scroller">
          ...
          <div className="hero" onClick={cyclePhoto}>
            ...
            <div className="edit-recency-lbl">{relativeTime(profile.lastUpdated)}</div>
            <div className="scrim"/>
          ...
          <div className="content">
            {/* Change 11 (moved to top per v17 Change 2): Completeness bar */}
            <div className={`completeness-bar-wrap${completenessPct===100?' completeness-wrap-done':''}`}>
              ...
              <div className="completeness-steps">
                {completenessSteps.map(s => (
                  <div key={s.id} className="completeness-step">
                    <div className={`step-check${s.done?' done':''}`}>
                      <svg viewBox="0 0 12 12"><polyline points="2,6 5,9 10,3"/></svg>
                    </div>
                    <span className={`step-text${s.done?' done':''}`}>{s.label}</span>
                    {!s.done && <span className="step-cta" onClick={s.cta}>{s.ctaLabel} →</span>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="intent-card">
              ...
              <div className="i-hl"><div className="i-dot"/><span className="i-lbl">{isRomantic?'Meet someone':'Build'}</span></div>
              <div className="i-pills">
                {isRomantic ? (
                  <>
                    <span className="ip-p">Something serious</span>
                    <span className="ip-s">Curious & kind</span>
                    <span className="ip-s">Long walks, real talks</span>
                  </>
                ) : (
                  <>
            ...
            {/* Change 13 (moved to top of About me per v17 Change 3): Romantic card — only when intent is romantic. v18 Change 3: now uses howup fields. */}
            {isRomantic && (() => {
              // Compute visible howup fields with values
              const visibleHowup = HOWUP_FIELDS.filter(f =>
                profile.howupVis[f] && profile.howupVals[f] && profile.howupVals[f].trim()
              );
              const previewName = visibleHowup.length > 0 ? profile.howupVals[visibleHowup[0]] : 'Tap to see';
              const previewRest = visibleHowup.slice(1, 4).map(f => profile.howupVals[f]).join(' · ');
              return (
                <div className="p-card pc-romantic">
                  <div className="p-header" onClick={() => setRomanticOpen(o => !o)}>
                    <div className="p-ico">
                    ...
                    <div className="p-txt">
                      ...
                      {!romanticOpen && <>
                        <div className="p-preview-name">{previewName}</div>
                        <div className="p-preview-rest">{previewRest}</div>
                      </>}
                    </div>
                  ...
                  <div className={`p-body${romanticOpen?' open':''}`}>
                    <div className="howup-expanded">
                      <div className="howup-grid">
                        {visibleHowup.map(f => (
                          <div key={f} className="howup-row">
                            <span className="howup-label">{HOWUP_LABELS[f]}</span>
                            <span className="howup-val">{profile.howupVals[f]}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            {/* v18 Change 2 + v19 Change 1A: Work card — moved here, between Music and Hobbies */}
            <div className="p-card pc-work">
              <div className="p-header" onClick={() => setWorkOpen(o => !o)}>
                <div className="p-ico">
                ...
                <div className="p-txt">
                  <div className="p-label">Work</div>
                  {!workOpen && <>
                    <div className="p-preview-name">{(() => {
                      const parts = (profile.workCurrent || '').split(',');
                      return parts[parts.length-1].trim() || profile.workCurrent || '—';
                    })()}</div>
                    <div className="p-preview-rest">{[profile.workCurrent, profile.workPrev].filter(Boolean).join(' · ')}</div>
                  </>}
                </div>
                <div className={`p-chev${workOpen?' open':''}`}>
                  <svg width="12" height="7" viewBox="0 0 12 7" fill="none" stroke="#8C4E14" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M1 1l5 5 5-5"/></svg>
                </div>
              </div>
                ...
                <div className="work-expanded">
                  {profile.workCurrent && (
                    <div className="work-item">
                      ...
                      <div>
                        <div className="work-title">{profile.workCurrent}</div>
                        {profile.workIndustry && <div className="work-org">{profile.workIndustry}</div>}
                  ...
                  {profile.workPrev && (
                    <div className="work-item">
                      <div className="work-dot"/>
                      <div>
                        <div className="work-title">{profile.workPrev}</div>
                        <span className="work-badge">Previous / Incoming</span>
                      </div>
                    </div>
                  )}
                </div>
        ...
        {/* ── Safari bottom bar ── */}
        <div className="sbot">
          <div className="sbtn dim"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6"/></svg></div>
          <div className="sbtn"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6"/></svg></div>
          <div className="sbtn"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg></div>
          <div className="sbtn"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg></div>
          <div className="sbtn"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg></div>
        </div>
        ...
        <div className={`edit-panel${editOpen?' open':''}`}>
          ...
          {editData && (
            <div className="edit-body">
              ...
              {/* Change 13A: Intent toggle */}
              <div className="edit-section">
                <div className="edit-section-label">Here to</div>
                <div className="intent-toggle-row">
                  {[['build','Build'],['romantic','Meet someone']].map(([val,label]) => (
                    <button key={val} className={`intent-toggle-btn ${editData.intent===val?'active':'inactive'}`}
                      onClick={() => setEditData(d => ({...d, intent: val}))}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              {/* v18 Change 2: Work edit section */}
              <div className="edit-section">
                <div className="edit-section-label">Work</div>
                <div className="edit-field">
                  <div className="edit-label">Current role / company</div>
                  <input className="edit-input" type="text" value={editData.workCurrent}
                    placeholder="e.g. Co-founder, Supercharged"
                    onChange={e => setEditData(d => ({...d, workCurrent: e.target.value}))}/>
                </div>
                <div className="edit-field">
                  <div className="edit-label">Previous or incoming role</div>
                  <input className="edit-input" type="text" value={editData.workPrev}
                    placeholder="e.g. Incoming SBA, McKinsey"
                    onChange={e => setEditData(d => ({...d, workPrev: e.target.value}))}/>
                </div>
                <div className="edit-field" style={{marginBottom:0}}>
                  <div className="edit-label">Industry or focus</div>
                  <input className="edit-input" type="text" value={editData.workIndustry}
                    placeholder="e.g. Tech, Finance, Research"
                    onChange={e => setEditData(d => ({...d, workIndustry: e.target.value}))}/>
                </div>
              </div>
              ...
              <div className="edit-section">
                ...
                <TagInput type="music" chips={editData.music} setChips={v => setEditData(d=>({...d,music:typeof v==='function'?v(d.music):v}))} placeholder="Add an artist..." chipClass="music"/>
              </div>
              ...
              <div className="edit-section">
                ...
                <TagInput type="hobbies" chips={editData.hobbies} setChips={v => setEditData(d=>({...d,hobbies:typeof v==='function'?v(d.hobbies):v}))} placeholder="Add an interest..." chipClass="hobbies"/>
              </div>
              ...
              <div className="edit-section">
                ...
                <TagInput type="socs" chips={editData.campus} setChips={v => setEditData(d=>({...d,campus:typeof v==='function'?v(d.campus):v}))} placeholder="Add a society or club..." chipClass="campus"/>
              </div>

              {/* v19 Change 3: How you show up — tappable field rows that open picker sheet */}
              {editData.intent === 'romantic' && (
                <div className="edit-section">
                  ...
                  <p style={{fontSize:11,color:'#AFAFAF',marginBottom:6,lineHeight:1.5,fontFamily:"'DM Sans',sans-serif"}}>
                    Tap a field to edit. Toggle visibility inside each field.
                  </p>
                  <div>
                    {HOWUP_FIELDS.map(f => {
                      const val = editData.howupVals[f] || '';
                      const isVis = editData.howupVis[f];
                      const isEmpty = !val || !val.trim();
                      return (
                        <div key={f} className="howup-field-row" onClick={() => openHowUpPicker(f)}>
                          <div className="howup-field-left">
                            <span className="howup-field-label">{HOWUP_LABELS[f]}</span>
                            <span className={`howup-field-val${isEmpty?' empty':''}`}>
                              {isEmpty ? `Add ${HOWUP_LABELS[f].toLowerCase()}` : val}
                            </span>
                          </div>
                          <div className="howup-field-right">
                            <span className={`howup-vis-badge ${isVis?'visible':'hidden-val'}`}>
                  ...
                  <p style={{fontSize:10,color:'#AFAFAF',marginTop:10,lineHeight:1.5,fontFamily:"'DM Sans',sans-serif"}}>
                    Only visible to others who have also set a romantic intent.
                  </p>
                </div>
              )}
            </div>
          ...
          {/* v19 Change 3D: How you show up — picker sheet (slides from right, z-index 75) */}
          <div className={`howup-picker${howupPicker.open?' open':''}`}>
            ...
            <div className="howup-picker-body">
              {howupPicker.field && (() => {
                ...
                const opts = HOWUP_OPTIONS[howupPicker.field] || [];
                return (
                  <>
                    ...
                    <div className="howup-options-wrap">
                      {opts.map(o => (
                        <div key={o}
                          className={`howup-option${howupPicker.selected.includes(o)?' selected':''}`}
                          onClick={() => toggleHowUpOption(o)}>
                          {o}
                        </div>
                      ))}
                    </div>
                    <div className="howup-other-label">Something else</div>
                    <input className="howup-other-input" type="text"
                      placeholder="Type your own answer..."
                      value={howupPicker.other}
                      onChange={e => setHowupPicker(p => ({ ...p, other: e.target.value }))}/>
                    <div className="howup-picker-vis-row">
                      <span className="howup-picker-vis-label">Visible on profile</span>
                      <button className={`howup-picker-vis-toggle ${howupPicker.visible?'on':'off'}`}
                        onClick={toggleHowUpPickerVis}/>
                    </div>
                  </>
                );
              })()}
