When considering appwrite interaction, use web search for latest documentation, recalling that it now uses a relational tablesdb structure, not the original collections structure.
Appwrite
...
bin/ -- CLI entry points: worker-_ (14 workers), schedule-_, queue-\*, plus doctor, install, migrate, realtime, upgrade, ssl, vars, maintenance, interval, specs, sdks, etc.
...
HTTP endpoint nesting reflects the URL path. Sub-resources get subdirectories. For example, within the Functions module: Http/Deployments/Template/Create.php -> POST /v1/functions/:functionId/deployments/template
